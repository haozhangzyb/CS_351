...
var rocketLoaction = [-0.5, -0.1, 0.0];
var rocketRate = 20.0;

var mouseDrag = [0.0, 0.0];
var WindmillLocation = [0.5, 0.7, 0.0];
var wmRate = 45.0;
...
function drawAll() {
  ...
  DrawRocketHeadAndBody(Tetra_g_modelMatrix, Tetra_u_ModelMatrix);
  DrawRocketEngine(Tetra_g_modelMatrix, Tetra_u_ModelMatrix);    

  DrawWindmill();
}

function DrawRocketHeadAndBody(Tetra_g_modelMatrix, Tetra_u_ModelMatrix) {
	//-------Draw Spinning Tetrahedron
	// Tetra_g_modelMatrix.setTranslate(-0.2, 0.4, 0.0); // 'set' means DISCARD old matrix,
  Tetra_g_modelMatrix.setTranslate(rocketLoaction[0], rocketLoaction[1], rocketLoaction[2]);
...
function DrawRocketEngine(Tetra_g_modelMatrix, Tetra_u_ModelMatrix, x, y, z) {
  var c30 = Math.sqrt(0.75); // == cos(30deg) == sqrt(3) / 2
  var sq2 = Math.sqrt(2.0);
  var x = [c30, 0.0, -c30];
  var y = [-0.5, 1.0, -0.5];
...
function DrawWindmill() {
  ...
  g_modelMatrix.setTranslate(WindmillLocation[0], WindmillLocation[1], WindmillLocation[2]); // 'set' means DISCARD old matrix,
  // (drawing axes centered in CVV), and then make new
  // drawing axes moved to the lower-left corner of CVV.
  g_modelMatrix.scale(1, 1, -1); // convert to left-handed coord sys
  // to match WebGL display canvas.
...
function myMouseMove(ev) {
  ...
  mouseDrag = [g_xMdragTot, g_yMdragTot];
  // Report new mouse position & how far we moved on webpage:
  document.getElementById("MouseAtResult").innerHTML =
    "Mouse At: " + x.toFixed(g_digits) + ", " + y.toFixed(g_digits);
  document.getElementById("MouseDragResult").innerHTML =
    "Mouse Drag: " +
    (x - g_xMclik).toFixed(g_digits) +
    ", " +
...
function myMouseUp(ev) {
  ...
  document.getElementById("MouseAtResult").innerHTML =
    "Mouse At: " + x.toFixed(g_digits) + ", " + y.toFixed(g_digits);
  console.log(
    "myMouseUp: g_xMdragTot,g_yMdragTot =",
    g_xMdragTot.toFixed(g_digits),
    ",\t",
    g_yMdragTot.toFixed(g_digits)
  );
}
